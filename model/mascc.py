...
#import os
#import logging
#from utils.util import load_prompt, normalize, load_cbt_technique_info, clean_json_response
...
#    return content
#
############### Supervisor System Integration Start ##############
#SUPERVISOR_ROLES = [
#    "Empathy",
#    "IdentifyCognitiveDistortion",
#    "ChallengeCognitiveDistortion",
#    "ProvideStrategy",
#    "Encouragement"
#]
#
#def call_supervisor_llm(role, prompt, model="gpt-4o-mini", temperature=0.7):
#    full_prompt = f"[ROLE: {role}]\n{prompt}"
#    return call_llm(full_prompt, model=model, temperature=temperature)
#
#def generate_draft_response(latest_dialogue, model, temperature):
#    draft_prompt = load_prompt("draft_response.txt").replace("[Dialogue]", latest_dialogue)
#    return call_llm(draft_prompt, model=model, temperature=temperature)
#
#def revise_with_supervisors(draft, cd_to_treat, retrieved_context, model, temperature):
#    advices = []
#    for role in SUPERVISOR_ROLES:
#        prompt = load_prompt(f"supervisor_{role.lower()}.txt")
#        filled_prompt = (
#            prompt
#            .replace("[Draft Response]", draft)
#            .replace("[Distortion]", cd_to_treat)
#            .replace("[Memory]", retrieved_context)
#        )
#        advice = call_supervisor_llm(role, filled_prompt, model, temperature)
#        advices.append((role, advice))
#    return advices
#
#def compose_final_response(draft, advice_list, model, temperature):
#    advice_text = "\n".join([f"{role}: {advice}" for role, advice in advice_list])
#    final_prompt = load_prompt("finalize_response.txt")
#    filled = (
#        final_prompt
#        .replace("[Draft Response]", draft)
#        .replace("[Supervisor Advices]", advice_text)
#    )
#    return call_llm(filled, model=model, temperature=temperature)
############### Supervisor System Integration End ##############
#
...
#
#    latest_dialogue = f"Counselor: {counselor_utterance}\nClient: {client_utterance}"
#    draft = generate_draft_response(latest_dialogue, args.model, args.temperature)
#    advice_list = revise_with_supervisors(draft, cd_to_treat, retrieved_context, args.model, args.temperature)
#    final_response = compose_final_response(draft, advice_list, args.model, args.temperature)
#
